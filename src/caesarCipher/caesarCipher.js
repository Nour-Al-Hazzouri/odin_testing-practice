...
export default function caesarCipher(string, key) {
  if (!string || !key) return "Ensure string & key availability";
  else if (!isEnglish(string)) return "Incompatible string";
  else if (!isEnglishNumber(key)) return "Incompatible key";

  const splitString = string.split("");
  const shiftedString = [];
  for (let i = 0; i < string.length; i++) {
    if (/[^a-zA-Z]/.test(splitString[i])) shiftedString[i] = splitString[i];
